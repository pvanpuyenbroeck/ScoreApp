...
const MatchDetailsMenu = (props) => {
    ...
    const joinClickedHandler = (match, addOrRemove) => {
        ...
        const GetMatchDetailsMenu = () => {
            ...
            typeof props.team.Seasons[props.selectedSeason] === 'undefined'){
            return null;
            }
                    ...
                    return (
                        <DetailsContainer closeContainer={props.closeAllContainers} >
                            <div className={classes.MatchNavButtonContainer}>
                                <div
                                    onClick={props.matchDetailsToggleHandler}
                                    className={[classes.Button, classes.ButtonMatch].join(' ')}
                                    style={{opacity:props.matchDetailsToggle ? '0.5' : '1'}}
                                    >Match</div>
                                <div
                                    onClick={props.matchDetailsToggleHandlerTrue}
                                    className={[classes.Button, classes.ButtonMembers].join(' ')}
                                    style={{opacity:!props.matchDetailsToggle ? '0.5' : '1'}}                                
                                    >Deelnemers</div>
                            </div>
                            {!props.matchDetailsToggle ? <MatchDetails
